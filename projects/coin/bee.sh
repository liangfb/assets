...
aws dynamodb describe-table --table-name swarmaddr --region $region
if [ "$?" -gt 0 ]
then
    ...
    aws dynamodb wait table-exists --table-name swarmaddr
fi
...
pass=$(cat $passfile)
aws dynamodb put-item --table-name swarmaddr --item "{\"addr\": {\"S\": \"$addr\"}, \"privatekey\": {\"S\": \"$escapedkey\"}, \"password\": {\"S\": \"$pass\"}}"
